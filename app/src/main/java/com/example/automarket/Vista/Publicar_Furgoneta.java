...
public class Publicar_Furgoneta extends AppCompatActivity {
    ...
    private static final String URL_PUBLICAR = Utils.IP + "publicar_furgoneta.php";
    private String vendedorId;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.publicar_furgoneta);
    ...
    private void publicarFurgoneta() {
        ...
        if (marca.isEmpty() || modelo.isEmpty() || anio.isEmpty() || kilometraje.isEmpty()
                || capacidadCarga.isEmpty() || precio.isEmpty() || descripcion.isEmpty()) {
            Toast.makeText(this, "Por favor, complete todos los campos", Toast.LENGTH_SHORT).show();
            return;
        }

        Log.d("PublicarFurgoneta", "Enviando: " + marca + ", " + modelo + ", " + anio +
                ", " + kilometraje + ", " + capacidadCarga + ", " + precio + ", " + descripcion +
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, URL_PUBLICAR,
                response -> {
                    ...
                    if (response.trim().equalsIgnoreCase("success")) {
                        Toast.makeText(this, "Furgoneta publicada con éxito", Toast.LENGTH_SHORT).show();
                        finish();
                    } else {
